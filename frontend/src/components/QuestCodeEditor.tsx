...
import { fetchNextQuest } from '@/lib/api';
...
interface QuestCodeEditorProps {
  questId: string;
  userId: string;
  starterCode: string;
  language: string;
...
export default function QuestCodeEditor({ questId, userId, starterCode, language, currentLanguageKey }: QuestCodeEditorProps) {
    ...
    const fetchSubmissions = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/submissions/user/${userId}?questId=${questId}`
        );
        if (response.ok) {
  ...
  // Check for next quest when a 100% submission is found
  useEffect(() => {
    const hasPassedSubmission = submissions.some(submission => submission.status === 'PASSED');
    if (hasPassedSubmission && !nextQuest && !isLoadingNext) {
      checkForNextQuest();
    }
  }, [submissions]);

  const checkForNextQuest = async () => {
    setIsLoadingNext(true);
    try {
      const next = await fetchNextQuest(currentLanguageKey, questId);
      setNextQuest(next);
    } catch (error) {
      console.error('Failed to fetch next quest:', error);
    } finally {
  ...
  const handleNextQuest = () => {
    if (nextQuest) {
      router.push(`/quests/${currentLanguageKey}/${nextQuest.id}`);
    } else {
      // No more quests, go back to quests page
      router.push(`/quests/${currentLanguageKey}`);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {submissions.some(s => s.status === 'PASSED') && (
        <div className="rounded-2xl border border-solid border-black/[.08] dark:border-white/[.145] p-6">
          <h3 className="font-semibold text-lg mb-4 text-green-700 dark:text-green-300">🎉 Quest Completed!</h3>
          {isLoadingNext ? (
            <div className="flex items-center gap-2 text-black/70 dark:text-white/70">
              <Loader2 className="h-4 w-4 animate-spin" />
              Looking for next quest...
            </div>
          ) : nextQuest ? (
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={() => router.push(`/quests/${currentLanguageKey}`)}
            ...
            <div className="space-y-3">
              <p className="text-sm text-black/70 dark:text-white/70">
                Congratulations! You've completed all quests for this language.
              </p>
              <button
                onClick={() => router.push(`/quests/${currentLanguageKey}`)}
