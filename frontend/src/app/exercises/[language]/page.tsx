...
export default async function ExercisesLanguagePage({ params }: PageProps) {
  ...
  let exercises: any[] = [];
  try {
    exercises = await fetchExercises(language);
  } catch (error) {
    ...
    <div className="min-h-screen min-w-[768px]">
      ...
      <main className="ml-64 p-8">
        <div className="text-responsive-xl font-semibold mb-2">
          Exercises {meta.label}
        </div>
        <p className="text-sm opacity-70 mb-8">
          Practice {meta.label} programming with hands-on coding challenges
        </p>
        ...
        {exercises.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm opacity-70">Coding exercises for {meta.label} are coming soon!</p>
          </div>
        ) : (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {exercises.map((exercise) => (
              <div key={exercise.id} className="rounded-2xl border border-solid border-black/[.08] dark:border-white/[.145] p-4 md:p-5 lg:p-6 hover:bg-[#f2f2f2] dark:hover:bg-[#1a1a1a] transition-colors flex flex-col h-full">
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-semibold text-base min-h-[3rem] flex items-start flex-1">{exercise.title}</h3>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs opacity-70">Exercise</span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-1 mb-4">
                  {exercise.tags.slice(0, 2).map((tag: any) => (
                    <span
                      key={tag.id}
                      className="px-2 py-1 text-xs rounded bg-black/[.08] dark:bg-white/[.12]"
                    >
                      {tag.name}
                    </span>
                  ))}
                  {exercise.tags.length > 2 && (
                    <span className="text-xs opacity-70">+{exercise.tags.length - 2}</span>
                  )}
                </div>

                <div className="flex justify-end mt-auto">
                  <Link
                    href={`/exercises/${language}/${exercise.id}`}
